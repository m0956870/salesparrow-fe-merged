...
import { getEmpPartyMapping, deleteEmpPartyMapping } from "../../../api/mappingAPI"
import fetchAllEmployee from "../../../api/employeeAPI";
import getStateFunc from "../../../api/locationAPI";


const EmpPartyMappingListing = () => {
    ...
    const [empPartyListing, setempPartyListing] = useState([])
    ...
    useEffect(() => {
        ...
        getEmpPartyMappingFunc(filterData)
    }, [pageCount])

    // console.log("empPartyListing", empPartyListing)
    // console.log("editDialogBoxData", editDialogBoxData)

    async function getEmpPartyMappingFunc(filterData) {
        setisLoading(true)

        let { data } = await getEmpPartyMapping(filterData)
        if (data.status) {
            setempPartyListing(data.result)
    ...
    const topFilterHandleInput = async (e) => {
        setfilterData({ ...filterData, [e.target.name]: e.target.value })
        getEmpPartyMappingFunc({ ...filterData, [e.target.name]: e.target.value })
    }
    ...
    const deletEmpPartyMappingFunc = async () => {
        // return console.log(editDialogBoxData)

        setisLoading(true)
        let { data } = await deleteEmpPartyMapping({ emp_id: editDialogBoxData.emp_id })
        if (data.status) {
            toast.success("Deleted Successfully!")
            getEmpPartyMappingFunc()
        ...
        <div className="container">
            <div className="beat_heading">
                <div className="beat_left">
                    ...
                    <div className="title">Employee / Party Mapping Listing</div>
                </div>
                <div className="beat_right">
                    {/* <div className="add_btn" onClick={() => navigate("/add_employee_party_mapping")}>
                        Add New
                    </div> */}
                </div>
            </div>
                    ...
                    <Table sx={{ minWidth: 700 }} aria-label="customized table">
                        <TableHead>
                            <TableRow>
                                ...
                                <StyledTableCell>Assigned Party Names</StyledTableCell>
                                <StyledTableCell>Actions</StyledTableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {empPartyListing?.map((row, i) => (
                                <StyledTableRow key={i} >
                                    ...
                                    <StyledTableCell>
                                        <div className="map_listing_assigned_col">
                                            ...
                                            <div>
                                                <NavLink to={`/emp_assigned_parties/${row.emp_id}`} >See all</NavLink>
                                            </div>
                                        </div>
                                    </StyledTableCell>
                                    <StyledTableCell>
...
export default EmpPartyMappingListing