...
import fetchAllEmployee, { addEmpTarget, editEmpTarget } from "../../api/employeeAPI";
...
const EditEmployeeTarget = () => {
  ...
  useEffect(() => {
    ...
    setempAssignedParties(location.state.target)
  }, []);
  ...
  const empTargetFunc = async () => {
    ...
    let res = await editEmpTarget(reqObj);
    if (res.data.status) {
      toast.success("Employee Target Edited Successfully!");
      ...
      <div className="addbeat_container">
        <div className="addbeat_form">
          <div className="employee_target_inputs">
            ...
            >
              <option value="">{location.state.state_name}</option>
              {allState?.map((state) => (
                <option key={state.id} value={state.id}>{state.name}</option>
              ))}
            </select>
            <select value={empTarget.month} name="month" onChange={handleInput} >
              <option value="">Selected - {monthsArr[Number(location.state.month) - 1].name}</option>
              {monthsArr?.map(month => (
          ...
          <div className="employee_target_inputs">
            <select name="employee" onChange={emphandleInput}>
              <option value="">{location.state.employee_name}</option>
            ...
            <select value={empTarget.year} name="year" onChange={handleInput} >
              <option value="">Selected - {location.state.year}</option>
              {yearsArr?.map(year => (
...
export default EditEmployeeTarget;
