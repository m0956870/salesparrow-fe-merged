...
import { editClaimSatus, editClaimStatus, getClaimReport, getPrimaryOrders } from "../../api/reportsAPI";
...
const ClaimApproval = () => {
    ...
    useEffect(() => {
        claimReportFunc({ ...filterData, page: pageCount });
    }, [pageCount]);

    useEffect(() => {
        if (search !== "") {
            let ID = setTimeout(() => {
                claimReportFunc({ ...filterData, search })
            }, 1000);
    ...
    const claimReportFunc = async (filterData) => {
        setisLoading(true)
        if (!await isAllowed(COLLECTION_MANAGEMENT)) {
            toast.error("Module is not purchased!");
            return setisLoading(false);
        }
        let { data } = await getClaimReport(filterData)
        if (data.status) {
    ...
    const editClaimStatusFunc = async (e, id) => {
        setisLoading(true);
        let res = await editClaimSatus({ id, approval_status: e.target.value });
        console.log(res);
        if (res.data.status) {
            toast.success("Claim Report Edited Successfully!");
            return claimReportFunc(filterData);
        }
        console.log("Error");
    };
    ...
    const filterFunc = () => {
        claimReportFunc(filterData)
    }

    const topFilterHandleInput = (e) => {
        setfilterData({ ...filterData, [e.target.name]: e.target.value })
        claimReportFunc({ ...filterData, [e.target.name]: e.target.value })
    }
    ...
    const editStatusFunc = async (status, row, amt) => {
        ...
        setisLoading(true);
        let res = await editClaimStatus({ id: row?.id || currentReport?.id, approval_status: status, approved_amount: approvedAmount || amt });
        if (res.data.status) {
    ...
    // Export
    let settings = {
        fileName: "Claim Approval Report", // Name of the resulting spreadsheet
        ...
        let data = [
            {
                ...
                columns: [
                    ...
                    { label: "Claim Amount", value: (row) => row.claim_amount || "" },
                    { label: "Claim Type", value: (row) => row.claim_type || "" },
                    { label: "Date", value: (row) => row.claim_date || "" },
                    { label: "Description", value: (row) => row.description || "" },
                    { label: "Approved Amount", value: (row) => row.approval_amount || "" },
                ],
                content: claimReport
            },
        ]
    ...
    const TCComponent = ({ data }) => {
        ...
        } else if (col.type === "description_link") {
            return (
                <StyledTableCell>
                    <div onClick={() => {
                    ...
                    <select name="day"
                        onChange={(e) => {
                            ...
                            if (e.target.value === "Approved") {
                                setapprovedAmount(row?.claim_amount);
                                editStatusFunc(e.target.value, row, row?.claim_amount)
        ...
        <div className="container" >
            <div className="beat_heading">
                <div className="beat_left">
                    ...
                    <div className="title">Claim Approval</div>
                </div>
            ...
            <div className="tracking_tabs" style={{ marginBottom: "1.5rem" }}>
                <div className="tarcking_tab_left">
                    ...
                    <div
                        className="view_btn"
                        onClick={() => claimReportFunc(filterData)}
                    >
                        View
                    </div>
                </div>
                ...
                <div className="approve_popup_container">
                    <DialogContent className="cardpopup_content">
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                            <input
                                type="text"
                                value={currentReport?.claim_amount}
                                disabled
                                onChange={(e) => {
            ...
            >
                <DialogContent className="cardpopup_content">
                    <img className="attachment_image" src={currentReport?.document} alt="Attachment Image" />
                </DialogContent>
            </Dialog>
            <Dialog
                open={messagePopup}
            ...
            >
                <DialogContent className="cardpopup_content">
                    <div>{currentReport?.description}</div>
                </DialogContent>
            </Dialog>
        </div>
    );
