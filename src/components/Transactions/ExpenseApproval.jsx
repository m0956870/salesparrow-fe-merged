...
import { editExpenseStatus, approveExpenseReport } from "../../api/reportsAPI";
...
const ExpenseApproval = () => {
    ...
    useEffect(() => {
        if (filterData.employee_id !== "") getMonthlyExpFunc({ ...filterData, page: pageCount });
    }, [pageCount]);

    useEffect(() => {
        if (search !== "") {
            let ID = setTimeout(() => {
                if (filterData.employee_id !== "") getMonthlyExpFunc({ ...filterData, search })
            }, 1000);
    ...
    const getMonthlyExpFunc = async (filterData) => {
        ...
        setisLoading(true)
        approveExpenseReport(filterData).then(res => {
    ...
    const editStatusFunc = async (status, row, amt) => {
        if (Number(currentReport?.data?.total_claim_amount) < Number(approvedAmount)) return toast.error("Approval Amount Must Be Less Than Submitted Amount!")
        setisLoading(true);
        let res = await editExpenseStatus({ id: row?.report_id || currentReport?.report_id, approval_status: status, approved_amount: approvedAmount || amt });
        if (res.data.status) {
            toast.success("Report Edited Successfully!");
            setapproveAmtPopup(false)
            getMonthlyExpFunc(filterData)
            setisLoading(false)
            setapprovedAmount(0)
            return;
        }
    ...
    const topFilterHandleInput = (e) => {
        setfilterData({ ...filterData, [e.target.name]: e.target.value })
        getMonthlyExpFunc({ ...filterData, [e.target.name]: e.target.value })
    }
    const filterAndExportFunc = (type) => {
        ...
        if (type === "column_filter") return setfilterDivExtended(!filterDivExtended);
            ...
            return saveToPdf(pdfView, "Expanse Approval");
        }
    }
        ...
        <div className="container">
            <div className="beat_heading">
                <div className="beat_left">
                    ...
                    <div className="title">Expense Approval</div>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="">
                    ...
                    {empSelected === true && expReport?.length === 0 && (
                        <div className="no_data">
                            No Data
                        </div>
                    )}
                ...
                <div className="approve_popup_container">
                    <DialogContent className="cardpopup_content">
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                            <input
                                type="text"
                                value={currentReport?.sub_total_claim_amount}
                                disabled
                                onChange={(e) => {
            ...
            >
                <DialogContent className="cardpopup_content">
                    <img className="attachment_image" src={currentReport?.attachment} alt="Attachment Image" />
                </DialogContent>
            </Dialog>
            ...
            <div id="hide_pdf_container" ref={pdfView}>
                <div className="pdf_head_section">
                    <div className="pdf_head_left">
                        ...
                        <div className="pdf_selected_date">Expanse Approval ({reportSummery?.pdf_start_date || "NA"} - {reportSummery?.pdf_end_date || "NA"})</div>
                    </div>
                ...
                <Table sx={{ minWidth: 700 }} aria-label="customized table">
                    ...
                    <TableBody>
                        ...
                        <PDFStyledTableRow >
                            ...
                            {tableCols[5]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.total_TA}</PDFStyledTableCell>}
                            {tableCols[6]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.total_DA}</PDFStyledTableCell>}
                            {tableCols[7]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.total_night_hault}</PDFStyledTableCell>}
                            {tableCols[8]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.total_misc_amount}</PDFStyledTableCell>}
                            {tableCols[9]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.total_claim_amount}</PDFStyledTableCell>}
                            {tableCols[10]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.approved_amount}</PDFStyledTableCell>}
                        </PDFStyledTableRow>
                    </TableBody>
                </Table>
