...
const ShareParty = () => {
  ...
  const selectionBtnFunc = (type) => {
    ...
    if (type === "action") {
      ...
      if (selectedPartyArr.length === 0) return toast.error("Select Leads First!")
  ...
  const selectionAllBtnFunc = (type) => {
    ...
    if (type === "action") {
      setselectionBtn(type)
      allParties.map((elem ,id)=>{
        setselectedPartyArr([...selectedPartyArr, elem.id])
      })
  ...
  const selectionCheckboxFunc = (e, row) => {
    ...
    if (!selectedPartyArr.includes(row.id)) {
      setselectedPartyArr([...selectedPartyArr, row.id])
    } else {
      let filteredArr = selectedPartyArr.filter(_id => _id !== row.id)
      setselectedPartyArr(filteredArr)
    }
  }

  const handleMessage=()=>{
    if(selectedPartyArr.length){
        setsendMessagePopup(true)
        const selectePartyName =  allSent?allParties:allParties.filter((elem)=>{
          return selectedPartyArr.includes(elem.id);
        })
       setSentName(selectePartyName.map((elem)=>elem))
    }else{
      ...
      <div className="tracking_tabs" style={{ marginBottom: "1.5rem" }}>
        ...
        <div className="top_filter_section">
          ...
          <div className="new_add_btn_top_filter">
            ...
            {allParties?.length !== 0 && (
             <>
              <div  className="top_right_create_btn_icon" onClick={handleMessage} style={{ marginLeft: "0.8rem" }}>
                  Send
                </div>
         ...
         {selectPopup ?
        <div className='option_lists'style={{right:"10%" , top:"100%"}} >
                    ...
                    <StyledTableRow key={i} >
                      ...
                      {selectionBtn === "action" && (
                          <input
                            ...
                            checked={allRowSelect?true:selectedPartyArr.includes(row.id)}
                            type="checkbox"
