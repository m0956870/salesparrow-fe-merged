...
import fetchAllBrands, { addProduct, fetchAllCategory, fetchAllUnit } from "../../../api/productAPI";
...
const AddProduct = () => {
  ...
  const addProductFunc = async () => {
    ...
    if (!blankValidator(product.brand_id)) {
      return setError({
        ...error,
        brand_id: {
          status: true,
    ...
    if (!blankValidator(product.catagory_id)) {
      return setError({
        ...error,
        catagory_id: {
          status: true,
    ...
    if (!blankValidator(product.hsn_code)) {
      return setError({
        ...error,
        hsn_code: {
          status: true,
    ...
    try {
      let res = await addProduct(profilePic, allData);
      // console.log(res);
      if (res.data.status) {
  ...
  return (
    <div className="container">
      ...
      <div className="addbeat_container">
        ...
        <div onClick={() => addProductFunc()} className="btn changepass_btn">
          {btnLoading ? (
