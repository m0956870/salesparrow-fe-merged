...
import { getCusFeedbackReport, getRetailer, getRetailerType, getSSWiseReport } from "../../api/reportsAPI";
...
const CustomerFeedbackReports = () => {
  ...
  const [allRetailersType, setallRetailersType] = useState([]);

  const [cusFeedbackReport, setcusFeedbackReport] = useState([]);
  ...
  useEffect(() => {
    ...
    getRetailer().then(res => setallRetailers(res.data.result));
    getRetailerType().then(res => setallRetailersType(res.data.result));
  }, []);
  useEffect(() => {
    ssWiseReportFunc({ ...filterData, page: pageCount })
  }, [pageCount]);
  ...
  useEffect(() => {
    if (search !== "") {
      let ID = setTimeout(() => {
        ssWiseReportFunc({ ...filterData, search })
      }, 1000);
  ...
  const ssWiseReportFunc = async (filterData) => {
    ...
    let { data } = await getCusFeedbackReport(filterData)
    if (data.status) {
      setcusFeedbackReport(data.result)
  ...
  const filterAndExportFunc = (type) => {
    ...
    if (type === "column_filter") return setfilterDivExtended(!filterDivExtended);
      ...
      setfilterDivExtended(false);
      if (cusFeedbackReport.length < 1) return toast.error("Report list is empty!");
    ...
    <div className="container">
      <div className="beat_heading">
        ...
        <div className="beat_right employee_head">
          <select name="customer_id" className="select_btn new_state_select" onChange={filterFunc}
          >
            <option value="">Customer Type</option>
            {allRetailersType?.length === 0 && <option disabled value="">No Customer Type Found</option>}
            {allRetailersType?.map((retailer) => (
              <option value={retailer?._id}>{retailer?.customer_type}</option>
            ))}
          </select>
          <select name="retailer_id" className="select_btn new_state_select" onChange={filterFunc}>
            <option value="">Select Customer</option>
            {allRetailers.length === 0 && <option disabled value="">No Customer Found</option>}
            {allRetailers?.map((retailer) => (
              <option value={retailer?.id}>{retailer?.customerName}</option>
            ))}
          </select>
      ...
      <div className="tracking_tabs" style={{ marginBottom: "1.5rem" }}>
        <div className="tarcking_tab_left">
          ...
          {/* <select name="retailer_id" onChange={handleInput}>
            <option value="">All Retailers</option>
            {allRetailers.length === 0 && <option disabled value="">No Retailer Found</option>}
            {allRetailers?.map((retailer) => (
