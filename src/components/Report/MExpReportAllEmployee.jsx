...
const MExpReportAllEmployee = () => {
    ...
    useEffect(() => {
        getMonthlyExpFunc({ ...filterData, page: pageCount })
    }, [pageCount]);

    useEffect(() => {
        if (search !== "") {
            let ID = setTimeout(() => {
                getMonthlyExpFunc({ ...filterData, search })
            }, 1000);
    ...
    const getMonthlyExpFunc = async (filterData) => {
        ...
        getMonthlyExpReport(filterData).then(res => {
            ...
            //     if (key !== "") return { ...reportSummery, [key]: res.data[key] }
            // }))
    ...
    const exportFunc = () => {
        // console.log("expReport", expReport); return;
        if (expReport.length < 1) return toast.error("Report list is empty!");
    ...
    const TCComponent = ({ data }) => {
        ...
        if (col.type === "link") {
            return (
                <StyledTableCell>
                    <div onClick={() => navigate("/mexp_report_single_employee", { state: { ...row, start_date: filterData.start_date, end_date: filterData.end_date } })} className="map_listing_assigned_col">
        ...
        <div className="container">
            <div className="beat_heading">
                <div className="beat_left">
                    ...
                    <div className="title">Monthly Expense Report of All Employee</div>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="">
                    ...
                    {reportSummery && expReport?.length !== 0 && (
                        <div className="mattendance_summery_section">
                            ...
                            <div className="report_summery_body">
                                <div className="report_summery_left">
                                    ...
                                    <div className="summery_column">
                                        <div className="detail_title">Total Submit Amount:</div>
                ...
                <Table sx={{ minWidth: 700 }} aria-label="customized table">
                    ...
                    <TableBody>
                        ...
                        <PDFStyledTableRow >
                            ...
                            {tableCols[3]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.result?.total_TA}</PDFStyledTableCell>}
                            {tableCols[4]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.result?.total_DA}</PDFStyledTableCell>}
                            {tableCols[5]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.result?.total_night_hault}</PDFStyledTableCell>}
                            {tableCols[6]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.result?.total_misc_amount}</PDFStyledTableCell>}
                            {tableCols[7]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.result?.total_claim_amount}</PDFStyledTableCell>}
                            {tableCols[8]?.active && <PDFStyledTableCell style={{ fontSize: "1.2rem", fontWeight: "bold" }}>{reportSummery?.result?.approved_amount}</PDFStyledTableCell>}
                        </PDFStyledTableRow>
                    </TableBody>
                </Table>
...
export default MExpReportAllEmployee;